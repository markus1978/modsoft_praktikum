...
class CodeGenerator implements IGenerator {
	...
	dispatch def condition(StringCondition condition) {
		return switch(condition.operator) {
			case StringOperators.CONTAINS: ""
			case StringOperators.CONTAINSNOT: "!"
		} + '''result.getString("«condition.field.fieldDeclaration.name»").toLowerCase().contains(«condition.operand.toLowerCase»)'''
	}
